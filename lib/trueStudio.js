...
function createClient(proxyUrl) {
  ...
  if (proxyUrl && typeof proxyUrl === 'string' && proxyUrl.trim()) {
    const { buildTlsFingerprintAgent } = require('./tlsProxyAgent');
    ...
    httpsAgent = buildTlsFingerprintAgent(withScheme);
    httpAgent  = (scheme === 'socks' || scheme === 'socks4' || scheme === 'socks5' || scheme === 'socks5h')
      ? require('socks-proxy-agent').SocksProxyAgent && new (require('socks-proxy-agent').SocksProxyAgent)(withScheme)
      : new HttpProxyAgent(withScheme);
...
// ─────────────────────────────────────────────────────────────────────────────
// humanThink — context-aware "thinking" pause mimicking what a developer does
// between portal interactions. Each context reflects a real UI moment.
// ─────────────────────────────────────────────────────────────────────────────
const _THINK_DELAYS = {
  read_page:       [1000, 3000],  // scanning the app list
  click_button:    [250,  700],   // clicking a UI button
  fill_form:       [600,  1800],  // typing in a name field
  wait_load:       [700,  2000],  // waiting for page to respond
  think:           [1200, 3500],  // deciding what to do next
  error_recovery:  [2000, 5500],  // confused by an error, rethinking
  after_success:   [500,  1200],  // satisfied glance after creation succeeds
  navigate:        [600,  1800],  // clicking a sidebar link
  scroll:          [300,  900],   // scrolling the list
  micro:           [100,  350],   // tiny hesitation (mouse hover etc.)
};
async function humanThink(context = 'think', speedFactor = 1.0) {
  const [min, max] = _THINK_DELAYS[context] || _THINK_DELAYS.think;
  return _humanDelay(min, max, speedFactor);
}

// ─────────────────────────────────────────────────────────────────────────────
// simulatePreCreation — mimics what a real developer does just before clicking
// "New Application". Looks at the existing list, maybe scrolls, thinks a bit.
// Keeps Discord's behavior-analysis happy by showing normal usage patterns.
// ─────────────────────────────────────────────────────────────────────────────
async function simulatePreCreation({ token, client, netOpts, speedFactor = 1.0 }) {
  if (!client || !token) return;
  const h = (extra = {}) => ({
    Authorization: token,
    'X-Super-Properties': client.superPropsB64,
    'X-Context-Properties': CTX_DEV_PORTAL,
    Referer: client.currentPage || 'https://discord.com/developers/applications',
    ...extra,
  });

  // 1. Developer "reads" the app list — GET /applications like the portal does
  await humanThink('read_page', speedFactor);
  try {
    await client.http.get(`${API}/applications?with_team_applications=true`, { headers: h() });
  } catch (_) {}

  // 2. Simulated scroll / reading existing apps
  await humanThink('scroll', speedFactor);

  // 3. Occasionally fetch /users/@me as the portal does in background
  if (Math.random() < 0.6) {
    try { await client.http.get(`${API}/users/@me`, { headers: h() }); } catch (_) {}
    await humanThink('micro', speedFactor);
  }

  // 4. "Thinking" before clicking New Application button
  await humanThink('think', speedFactor);

  // 5. Navigate to the new-application form (simulates clicking the button)
  await navigateTo({ client, page: 'https://discord.com/developers/applications' });
  await humanThink('click_button', speedFactor);
}
...
class DiscordError extends Error {
  constructor(msg, { code, status, data, retryAfter, rateLimit, captchaSitekey, captchaRqdata, captchaRqtoken, captchaSessionId, captchaWall } = {}) {
    super(msg);
    this.code   = code   || '';
    ...
    if (captchaWall) this.captchaWall = true;
  }
}
  ...
  if (_isCaptcha(r.data) && _cap < 1 && typeof solveCaptcha === 'function') {
    ...
    if (sitekey) {
      try {
        ...
        // Preserve CAPTCHA_WALL flag — signals account is permanently captcha-walled
        const isCaptchaWall = e?.captchaWall === true || e?.code === 'CAPTCHA_WALL';
        throw new DiscordError('Captcha solver failed: ' + (e?.message || e), {
          code: isCaptchaWall ? 'CAPTCHA_WALL' : 'CAPTCHA_FAILED',
          status: r.status, data: r.data,
          captchaSitekey: sitekey, captchaRqdata: rqdata, captchaRqtoken: rqtoken, captchaSessionId: sessionId,
          captchaWall: isCaptchaWall,
        });
      }
    }
...
// ─────────────────────────────────────────────────────────────────────────────
async function simulateBrowsing({ token, netOpts, speedFactor = 1.0 }) {
  const client = netOpts?.client;
  if (!client) return;
  const sf = netOpts?.speedFactor ?? speedFactor;
  ...
  // 1. Navigate to channels (real client always starts here after login)
  await navigateTo({ client, page: 'https://discord.com/channels/@me' });
  await humanThink('wait_load', sf);

  // 2. Fetch own profile
  try { await client.http.get(`${API}/users/@me`, { headers: h() }); } catch (_) {}
  await humanThink('micro', sf);

  // 3. Fetch DM list — real client loads this immediately
  try { await client.http.get(`${API}/users/@me/channels`, { headers: h() }); } catch (_) {}
  await humanThink('scroll', sf);

  // 4. Fetch guild list — real client shows server sidebar
  try { await client.http.get(`${API}/users/@me/guilds?with_counts=true`, { headers: h() }); } catch (_) {}
  await humanThink('read_page', sf);

  // 5. Occasionally look at notifications (35% of the time)
  if (Math.random() < 0.35) {
    try { await client.http.get(`${API}/users/@me/mentions`, { headers: h() }); } catch (_) {}
    await humanThink('micro', sf);
  }

  // 6. Science telemetry — Discord expects this on every client session open
  try {
    await client.http.post(`${API}/science`, {
      events: [{ type: 'app_opened', properties: { platform: 'web', build_number: client.buildNumber } }],
    }, { headers: h({ 'Content-Type': 'application/json' }) });
  } catch (_) {}
  await humanThink('navigate', sf);

  // 7. Navigate toward developer tools (real developer's next step)
  await navigateTo({ client, page: 'https://discord.com/developers' });
  await humanThink('wait_load', sf);
}
...
async function accountHealthProbe({ token, netOpts = {} }) {
  ...
  // Probe WITHOUT solveCaptcha — we want to DETECT captcha walls, not solve them.
  // If Discord demands captcha on a health-check endpoint it means the account is
  // flagged / soft-banned and every real API call will also hit the same wall.
  const probeOpts = { ...netOpts, client, solveCaptcha: null };

  async function probe(name, url) {
    let r, hasCaptcha = false;
    try {
      r = await _req({
        method: 'GET', url, token, netOpts: probeOpts,
        extra: { 'X-Context-Properties': CTX_DEV_PORTAL },
      });
      // Even a 200 can carry captcha_key when Discord soft-flags the account
      hasCaptcha = _isCaptcha(r.data);
    } catch (e) {
      // CAPTCHA_REQUIRED / CAPTCHA_WALL / CAPTCHA_FAILED all mean captcha wall
      if (e?.code === 'CAPTCHA_REQUIRED' || e?.code === 'CAPTCHA_WALL' || e?.captchaWall ||
          /captcha/i.test(e?.message || '')) {
        const item = { name, status: 400, ok: false, retryAfter: 0, scope: null, global: false,
          code: 'CAPTCHA_WALL', message: 'Captcha required on health check — account is flagged', hasCaptcha: true };
        checks.push(item);
        return item;
      }
      // Other error (network, 401, 429) — surface it normally
      const item = { name, status: e?.status || 0, ok: false, retryAfter: 0, scope: null, global: false,
        code: e?.code || null, message: e?.message || String(e), hasCaptcha: false };
      checks.push(item);
      return item;
    }
    ...
    const item = { name, status: r.status, ok: r.status >= 200 && r.status < 300 && !hasCaptcha,
      retryAfter, scope, global, code: r.data?.code || null, message: r.data?.message || '', hasCaptcha };
    checks.push(item);
    return item;
  }
  ...
  const captchaWall = checks.find(c => c.hasCaptcha);
  const bad = captchaWall ||
    checks.find(c => c.status === 429) ||
    checks.find(c => c.status === 401) ||
    checks.find(c => c.status === 403) ||
    checks.find(c => !c.ok);
  let ok = !bad;
  let classification = ok ? 'ok' : 'unknown';
  let message = ok ? 'Account health looks OK' : (bad.message || `HTTP ${bad.status}`);
  if (bad) {
    if (bad.hasCaptcha || bad.code === 'CAPTCHA_WALL') {
      classification = 'captcha_wall';
      message = 'الحساب خلف حائط كابتشا — Discord يعتبره مشبوهاً ويطلب التحقق على كل طلب';
    } else if (bad.status === 429) {
      classification = bad.global ? 'global_rate_limited' : 'rate_limited';
      message = `Rate limited (${bad.scope || 'unknown scope'}), retry after ${bad.retryAfter || '?'}s`;
    } else if (bad.status === 401) {
      classification = 'invalid_token';
      message = 'Token expired/invalid — stop before making more requests';
    } else if (bad.status === 403) {
      classification = 'forbidden_or_locked';
      message = bad.message || 'Forbidden — account may be locked, phone-verified, or missing access';
    }
  }
...
function cloneClientWithProxy(baseClient, proxyUrl) {
  ...
  if (proxyUrl && typeof proxyUrl === 'string' && proxyUrl.trim()) {
    ...
    httpsAgent = buildTlsFingerprintAgent(withScheme);
    httpAgent  = (scheme === 'socks' || scheme === 'socks4' || scheme === 'socks5' || scheme === 'socks5h')
      ? new (require('socks-proxy-agent').SocksProxyAgent)(withScheme)
      : new HttpProxyAgent(withScheme);
    useCookieInterceptors = true;
  } else {
    const agents = _buildAgents(jar);
